...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (canLook == true)
        {
            LookAngle(new Vector2(Input.GetAxis("Mouse X") * sensitivityModifier.NewFloat(sensitivityX) * Time.deltaTime, Input.GetAxis("Mouse Y") * sensitivityModifier.NewFloat(sensitivityY) * Time.deltaTime));
        }
        #endregion
